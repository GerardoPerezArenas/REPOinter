...
public class GestionServiciosNISAE {
    ...
    private static final Logger LOG = Logger.getLogger(GestionServiciosNISAE.class);
...
//                }
    ...
    private String formatearRespuestaHHFF(net.lanbide.interoperability.otddff.beans.LanNISAESalida r) {
        StringBuilder sb = new StringBuilder();
        sb.append("Estado: ")
          .append(nvlObj(r.getCodigoEstado())).append(" - ").append(nvlObj(r.getLiteralEstado()));
        appendCampo(sb, "Resultado",               r.getResultado());
        appendCampo(sb, "Detalle",                 r.getDescripcionResultado());
        appendCampo(sb, "Estado secundario",       r.getCodigoEstadoSecundario());
        appendCampo(sb, "Motivos no cumple",       r.getMotivosNoCumple());
        appendCampo(sb, "Motivo error",            r.getMotivosError());
        appendCampo(sb, "Tiempo estimado respuesta", r.getTiempoEstimadoRespuesta());
        appendFechaHora(sb, r.getFechaHoraRespuestaPendiente());
        return sb.toString();
    }

    private void appendCampo(StringBuilder sb, String etiqueta, Object valor) {
        if (valor != null) {
            String s = String.valueOf(valor).trim();
            if (!s.isEmpty() && !"null".equals(s)) {
                sb.append("\n").append(etiqueta).append(": ").append(s);
            }
        }
    }

    private void appendFechaHora(StringBuilder sb, Object valor) {
        if (valor != null) {
            String s = String.valueOf(valor).trim();
            if (!s.isEmpty() && !"null".equals(s)) {
                if (s.contains("T")) {
                    String[] partes = s.split("T", 2);
                    sb.append("\n").append("Fecha consulta: ").append(partes[0]);
                    sb.append("\n").append("Hora consulta: ").append(partes[1]);
                } else {
                    sb.append("\n").append("Fecha/Hora consulta: ").append(s);
                }
            }
        }
    }

    private String nvlObj(Object o) {
        if (o == null) return "";
...
//                }
   ...
   private String prepararRespuestaServicioParaCampoSupleCVL(ResponseRestServiceCVL respuesta){
    ...
    void crearInteropVL(ResponseRestServiceCVL responseRestServiceCVL, RequestRestServiceCVL requestRestServiceCVL, int idRegistroLog, String valorDNI, String tipo_doc, String numeroExpediente, AdaptadorSQLBD adaptador) throws Exception {
        LOG.info("crearInteropVL - Begin () " + formatDate.format(new Date()) );
        Connection con = null;
        try {
            con = adaptador.getConnection();
            daoService.crearInteropVL(responseRestServiceCVL, requestRestServiceCVL, idRegistroLog, valorDNI, tipo_doc, numeroExpediente, con);
        }catch (Exception e) {
    ...
    private String formatearRespuestaTGSS(LanNISAESalida r) {
        StringBuilder sb = new StringBuilder();
        sb.append("Estado: ")
          .append(nvlObj(r.getCodigoEstado())).append(" - ").append(nvlObj(r.getLiteralEstado()));
        appendCampo(sb, "Resultado",                 r.getResultado());
        appendCampo(sb, "Detalle",                   r.getDescripcionResultado());
        appendCampo(sb, "Estado secundario",         r.getCodigoEstadoSecundario());
        appendCampo(sb, "Documento",                 r.getDocTitular());
        appendCampo(sb, "Motivos no cumple",         r.getMotivosNoCumple());
        appendCampo(sb, "Motivo error",              r.getMotivosError());
        appendCampo(sb, "Tiempo estimado respuesta", r.getTiempoEstimadoRespuesta());
        appendFechaHora(sb, r.getFechaHoraRespuestaPendiente());
    ...
    public String ejecutarConsultaIndividualTGSS(int codOrganizacion, InteropLlamadasServiciosNisae interopLlamadasServiciosNisae, String idProcoFlexia_LAN6, ExpedienteNisaeVO expedienteNisaeVO, AdaptadorSQLBD adaptador) throws Exception {
        String respuestaServicio = "";
        try {
            TerceroVO _tercero = expedienteNisaeVO.getTitularExpediente();
            if (_tercero != null) {
                String tipo_doc = MeLanbideInteropGeneralUtils.parsearTipoDocFlexiaToTipoDocEJIE_WS(_tercero.getTipoDoc());
                LOG.info("ejecutarConsultaIndividualTGSS - TipoDocumento Tercero Calculado " + tipo_doc);

                if (!traducirSeOponeInteroperabilidad(expedienteNisaeVO.getOposicionCertificadoPagoTGSS())) {
                    LOG.info("-- Nueva Peticion Individual TGSS");

                    net.lanbide.interoperability.tgss.servicios.LanTGSSServicio servicio =
                            new net.lanbide.interoperability.tgss.servicios.LanTGSSServicio(idProcoFlexia_LAN6);
                    LanNISAEEntrada lanNISAEEntrada = new LanNISAEEntrada();

                    int idRegistroLog = crearRegistroBBDDLogLlamadaTGSS(codOrganizacion, interopLlamadasServiciosNisae,
                            idProcoFlexia_LAN6, lanNISAEEntrada, expedienteNisaeVO.getObservaciones(),
                            expedienteNisaeVO.getNumeroExpediente(), adaptador);
                    ...
                    String cifResponsable  = ConfigurationParameter.getParameter(ConstantesMeLanbideInterop.RESPONSABLE_SERVICIO_NIF    + "TGSS", ConstantesMeLanbideInterop.FICHERO_PROPIEDADES);
                    String nombreResponsable = ConfigurationParameter.getParameter(ConstantesMeLanbideInterop.RESPONSABLE_SERVICIO_NOMBRE + "TGSS", ConstantesMeLanbideInterop.FICHERO_PROPIEDADES);
                    String consentimiento = ConfigurationParameter.getParameter(ConstantesMeLanbideInterop.CONSENTIMIENTO + "TGSS", ConstantesMeLanbideInterop.FICHERO_PROPIEDADES);
                    ...
                    lanNISAEEntrada.setNombreCompletoFuncionario(nombreResponsable);
                    lanNISAEEntrada.setNifFuncionario(cifResponsable);
                    ...
                    actualizarTextoJsonDatosEnviadosDocTGSSPeticionRegistroBBDDLogLlamada(idRegistroLog, lanNISAEEntrada, adaptador);

                    LanNISAESalida lanNISAESalida = servicio.consultaTGSS(lanNISAEEntrada);

                    if (lanNISAESalida != null) {
                        actualizarCamposSuplementariosNISAE_TGSS(codOrganizacion, expedienteNisaeVO.getNumeroExpediente(), lanNISAESalida, adaptador);
                        actualizarRegistroBBDDLogLlamadaTGSS(idRegistroLog, lanNISAESalida, expedienteNisaeVO.getObservaciones(), adaptador);
                        respuestaServicio = formatearRespuestaTGSS(lanNISAESalida);
                    } else {
                        respuestaServicio = "Error: Respuesta del servicio NISAE-TGSS recibida a null";
                        actualizarRegistroBBDDLogLlamadaTGSS(idRegistroLog, new LanNISAESalida(), "Respuesta del servicio recibida a Null", adaptador);
                    }
                } else {
                    respuestaServicio = "Servicio no invocado: hay oposicion a la llamada de los servicios de interoperabilidad TGSS para " + _tercero.getDoc();
                    crearRegistroBBDDLogLlamadaTGSS(codOrganizacion, interopLlamadasServiciosNisae, idProcoFlexia_LAN6,
                            new LanNISAEEntrada(), respuestaServicio, expedienteNisaeVO.getNumeroExpediente(), adaptador);
                }
            } else {
                respuestaServicio = "Error: Datos del tercero no recuperados";
            }
        } catch (Exception e) {
            LOG.error("Error en ejecutarConsultaIndividualTGSS para " + expedienteNisaeVO.getNumeroExpediente(), e);
            respuestaServicio = "Error excepcional: " + e.getMessage();
            throw e;
        }
        return respuestaServicio;
    }

       public String ejecutarConsultaIndividualHHFF(int codOrganizacion, InteropLlamadasServiciosNisae interopLlamadasServiciosNisae, String idProcoFlexia_LAN6, ExpedienteNisaeVO expedienteNisaeVO, AdaptadorSQLBD adaptador) throws Exception {
        String respuestaServicio = "";
        Connection con = null;
        try {
            con = adaptador.getConnection();
            ...
            if (_tercero != null) {
                String tipo_doc = MeLanbideInteropGeneralUtils.parsearTipoDocFlexiaToTipoDocEJIE_WS(_tercero.getTipoDoc());
                LOG.info("ejecutarConsultaIndividualHHFF - TipoDocumento Tercero Calculado " + tipo_doc);

                // Comprobamos si ya existe una peticion pendiente para este tercero y expediente (para evitar duplicados)
                if (expedienteNisaeVO.getCodigoEstadoSecundarioPeticion() == null || expedienteNisaeVO.getCodigoEstadoSecundarioPeticion().isEmpty()) {
                    List<InteropLlamadasServiciosNisae> pendientes = daoService.getPeticionesConsultasPendientesNISAExExpedienteDocTercero(codOrganizacion, expedienteNisaeVO.getNumeroExpediente(), _tercero.getDoc(), con);
                    if (pendientes != null && !pendientes.isEmpty()) {
                        InteropLlamadasServiciosNisae lastPending = pendientes.get(0);
                        expedienteNisaeVO.setIdPeticionPreviaBBDDLog(lastPending.getId());
                        expedienteNisaeVO.setCodigoEstadoSecundarioPeticion(lastPending.getCodigoEstadoSecundario());
                        LOG.info("ejecutarConsultaIndividualHHFF - Detectada peticion previa pendiente ID: " + lastPending.getId());
                    }
                }

                // Comprobamos que solo llamemos para CAPV y que no haya oposicion al llamar al servicio
                if (("01".equalsIgnoreCase(_tercero.getCodigoProvinciaDom())
                        ...
                        && !traducirSeOponeInteroperabilidad(expedienteNisaeVO.getOposicionObligacionesTributariasHF())) {

                    net.lanbide.interoperability.otddff.servicios.LanConsultaObligacionesTributariasServicio servicios = new net.lanbide.interoperability.otddff.servicios.LanConsultaObligacionesTributariasServicio(idProcoFlexia_LAN6);
                    net.lanbide.interoperability.otddff.beans.LanNISAEEntrada lan6ObligTribEntrada = new net.lanbide.interoperability.otddff.beans.LanNISAEEntrada();

                    // Si hay una peticion previa pendiente solo pasamos los datos de la Peticion
                    if (expedienteNisaeVO.getCodigoEstadoSecundarioPeticion() != null && !expedienteNisaeVO.getCodigoEstadoSecundarioPeticion().isEmpty()) {
                        LOG.info("-- Existe una peticion previa : " + expedienteNisaeVO.getCodigoEstadoSecundarioPeticion());
                        ...
                        lan6ObligTribEntrada.setIdPeticion(String.valueOf(idRegistroLogPadre));
                        lan6ObligTribEntrada.setCodigoEstadoSecundario(expedienteNisaeVO.getCodigoEstadoSecundarioPeticion());

                        int idRegistroLogHijo = crearRegistroBBDDLogLlamadaHHFF(codOrganizacion, interopLlamadasServiciosNisae, idProcoFlexia_LAN6, lan6ObligTribEntrada, expedienteNisaeVO.getObservaciones(), expedienteNisaeVO.getNumeroExpediente(), adaptador);
                        ...
                        if (lan6ObligTribSalida != null) {
                            actualizarCamposSuplementariosNISAE_HHFF(codOrganizacion, expedienteNisaeVO.getNumeroExpediente(), lan6ObligTribSalida, adaptador);
                            actualizarRegistroBBDDLogLlamadaHHFF(expedienteNisaeVO.getIdPeticionPreviaBBDDLog(), lan6ObligTribSalida, expedienteNisaeVO.getObservaciones(), adaptador);
                            if (!"0002".equalsIgnoreCase(lan6ObligTribSalida.getCodigoEstado())) {
                                actualizarSoloEstadoYdescripcionRegistroBBDDLogLlamada(idRegistroLogPadre, lan6ObligTribSalida.getCodigoEstado(), lan6ObligTribSalida.getLiteralEstado(), adaptador);
                            }
                            respuestaServicio = formatearRespuestaHHFF(lan6ObligTribSalida);
                        } else {
                            respuestaServicio = "Error: Respuesta del servicio NISAE recibida a null";
                            actualizarRegistroBBDDLogLlamadaHHFF(expedienteNisaeVO.getIdPeticionPreviaBBDDLog(), new net.lanbide.interoperability.otddff.beans.LanNISAESalida(), "Respuesta del servicio recibida a Null", adaptador);
                        }
                    } else {
                        LOG.info("-- Nueva Peticion Individual HHFF");
                        interopLlamadasServiciosNisae.setIdPeticionPadre(0);
                        int idRegistroLog = crearRegistroBBDDLogLlamadaHHFF(codOrganizacion, interopLlamadasServiciosNisae, idProcoFlexia_LAN6, lan6ObligTribEntrada, expedienteNisaeVO.getObservaciones(), expedienteNisaeVO.getNumeroExpediente(), adaptador);
                        expedienteNisaeVO.setIdPeticionPreviaBBDDLog(idRegistroLog);
                        ...
                        String cifResponsable = ConfigurationParameter.getParameter(ConstantesMeLanbideInterop.RESPONSABLE_SERVICIO_NIF + interopLlamadasServiciosNisae.getProcedimientoHHFF(), ConstantesMeLanbideInterop.FICHERO_PROPIEDADES);
                        String nombreResponsable = ConfigurationParameter.getParameter(ConstantesMeLanbideInterop.RESPONSABLE_SERVICIO_NOMBRE + interopLlamadasServiciosNisae.getProcedimientoHHFF(), ConstantesMeLanbideInterop.FICHERO_PROPIEDADES);

                        lan6ObligTribEntrada.setNombreCompletoFuncionario(nombreResponsable);
                        lan6ObligTribEntrada.setNifFuncionario(cifResponsable);
                        ...
                        lan6ObligTribEntrada.setApellido1(_tercero.getApellido1() != null && !_tercero.getApellido1().isEmpty() ? _tercero.getApellido1() : ("1".equalsIgnoreCase(_tercero.getTipoDoc()) || "3".equalsIgnoreCase(_tercero.getTipoDoc()) ? "N/A" : ""));
                        lan6ObligTribEntrada.setApellido2(_tercero.getApellido2() != null && !_tercero.getApellido2().isEmpty() ? _tercero.getApellido2() : ("1".equalsIgnoreCase(_tercero.getTipoDoc()) || "3".equalsIgnoreCase(_tercero.getTipoDoc()) ? "N/A" : ""));

                        actualizarTextoJsonDatosEnviadosDocTTHHPeticionRegistroBBDDLogLlamada(idRegistroLog, lan6ObligTribEntrada, adaptador);

                        net.lanbide.interoperability.otddff.beans.LanNISAESalida lan6ObligTribSalida = servicios.consultaObligacionesTributarias(lan6ObligTribEntrada);

                        if (lan6ObligTribSalida != null) {
                            actualizarCamposSuplementariosNISAE_HHFF(codOrganizacion, expedienteNisaeVO.getNumeroExpediente(), lan6ObligTribSalida, adaptador);
                            actualizarRegistroBBDDLogLlamadaHHFF(idRegistroLog, lan6ObligTribSalida, expedienteNisaeVO.getObservaciones(), adaptador);
                            respuestaServicio = formatearRespuestaHHFF(lan6ObligTribSalida);
                        } else {
                            respuestaServicio = "Error: Respuesta del servicio NISAE recibida a null";
                            actualizarRegistroBBDDLogLlamadaHHFF(idRegistroLog, new net.lanbide.interoperability.otddff.beans.LanNISAESalida(), "Respuesta del servicio recibida a Null", adaptador);
                        }
                    }
                } else {
                    respuestaServicio = "Servicio no invocado: titular con domicilio fuera de la CAPV o hay oposicion.";
                    crearRegistroBBDDLogLlamadaHHFF(codOrganizacion, interopLlamadasServiciosNisae, idProcoFlexia_LAN6, new net.lanbide.interoperability.otddff.beans.LanNISAEEntrada(), respuestaServicio, expedienteNisaeVO.getNumeroExpediente(), adaptador);
                }
            } else {
                respuestaServicio = "Error: Datos del tercero no recuperados";
            }
        } catch (Exception e) {
            LOG.error("Error en ejecutarConsultaIndividualHHFF para " + expedienteNisaeVO.getNumeroExpediente(), e);
            respuestaServicio = "Error excepcional: " + e.getMessage();
            throw e;
        } finally {
            try {
