            ...
            if (listExpedientes != null && listExpedientes.size() > 0) {
                ...
                for (ExpedienteNisaeVO vo : listExpedientes) {
                    ...
                    if (vo.getTitularExpediente() != null) {
                        ...
                        m_Log.info("GestionServiciosNISAEEjecucionConsPadron - TipoDocumento Tercero Calculado " + tipo_doc);
                    }
                    ...
                    // Registra en BD el Log Antes de llamar
                    m_Log.info("Escribir llamada en el m_Log INTEROP_SERVICIOS_NISAE_LOG");
                    int idRegLog = gestionServiciosNISAE.crearRegistroBBDDLogLlamadaPadron(codOrganizacion, interopLlamadasServiciosNisae, idProcoFlexia_LAN6, requestRestServicePadron, null, idPeticion, vo.getNumeroExpediente(), adaptador);
                    try {
                    ...
                    ResponseRestServicePadron responseRestServicePadron = null;
                    try {
                        ...
                        responseRestServicePadron = gson.fromJson(responseJSON.toString(), ResponseRestServicePadron.class);

                        m_Log.info("Actualizar llamada en el m_Log INTEROP_SERVICIOS_NISAE_LOG");
                        gestionServiciosNISAE.actualizarRegistroBBDDLogLlamadaPadron(idRegLog, responseRestServicePadron, conRS.getResponseCode() + " - " + conRS.getResponseMessage(), adaptador);
                        ...
                        gestionServiciosNISAE.actualizarRegistroBBDDLogLlamadaPadron(idRegLog, responseRestServicePadron, conRS.getResponseCode() + " - " + conRS.getResponseMessage() + " : "+respuestaServicio, adaptador);
                    }
                }
            } else {
